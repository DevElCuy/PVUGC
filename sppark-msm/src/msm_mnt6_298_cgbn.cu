// MNT6-298 CGBN-Based MSM Implementation
...
// Error codes
#define MNT6_MSM_SUCCESS 0
#define MNT6_MSM_ERROR_AFFINE_LAYOUT -1
#define MNT6_MSM_ERROR_SCALAR_LAYOUT -2
#define MNT6_MSM_ERROR_CUDA_RUNTIME -3
#define MNT6_MSM_ERROR_TIMEOUT -4

// CGBN parameters class (following CGBN sample pattern)
class mnt6_cgbn_params_t {
public:
...
// MNT6-298 base field modulus (Fq, 298 bits) == MNT4-298 Fr
// Little-endian u32[10] representation
__device__ __constant__ uint32_t MNT6_298_P_DEVICE[10] = {
    0x00000001, 0xbb4334a4, 0x925d6ad3, 0xfb494c07, 0x5cf44194,
    0xcaeec963, 0xa7b0548e, 0xa266249d, 0xf7bcd473, 0x000003bc
};

// MNT6-298 scalar field order (Fr, 298 bits) == MNT4-298 Fq
__device__ __constant__ uint32_t MNT6_298_R_DEVICE[10] = {
    0x71660001, 0xc90cd65a, 0x51200e12, 0x41a9e35e, 0x5d1330ea,
    0xcaeec963, 0xa7b0548e, 0xa266249d, 0xf7bcd473, 0x000003bc
};

// Curve parameter a for MNT6-298 G1: a = 11
#define MNT6_CURVE_A 11
...
/**
 * MNT6-298 MSM Class (following CGBN sample pattern)
 *
...
public:
  ...
  // Constructor
  __device__ __forceinline__ mnt6_msm_t(cgbn_monitor_t monitor,
   ...
   *
   * XYZZ Doubling Formulas for y^2 = x^3 + ax + b (MNT6-298 has a=11):
   * A = Y1^2
   * V = 4*A
   * U = 2*Y1
   * W = U*V = 8*Y1*A
   * S = X1*V
   * M = 3*X1^2 + a*ZZ1^2  (for a=11: M = 3*X1^2 + 11*ZZ1^2)
   *
   * X3 = M^2 - 2*S
  ...
  __device__ __forceinline__ void point_double(
    ...
    bn_t A, V, U, W, S, M, temp, a_term, zz1_sq;
    ...
    // M = 3*X1^2 + a*ZZ1^2  (MNT6-298 has a=11)
    field_mul(M, X1, X1, P);
    field_add(temp, M, M, P);
    field_add(M, M, temp, P);  // M = 3*X1^2

    // Add a*ZZ1^2 term (a=11 for MNT6)
    // Compute 11*ZZ1^2 = 8*ZZ1^2 + 2*ZZ1^2 + ZZ1^2
    field_mul(zz1_sq, ZZ1, ZZ1, P);  // ZZ1^2
    field_add(a_term, zz1_sq, zz1_sq, P);  // 2*ZZ1^2
    field_add(temp, a_term, a_term, P);  // 4*ZZ1^2
    field_add(a_term, temp, temp, P);  // 8*ZZ1^2
    field_add(temp, zz1_sq, zz1_sq, P);  // 2*ZZ1^2
    field_add(a_term, a_term, temp, P);  // 10*ZZ1^2
    field_add(a_term, a_term, zz1_sq, P);  // 11*ZZ1^2
    field_add(M, M, a_term, P);  // M = 3*X1^2 + 11*ZZ1^2
 ...
 *
 * Threading: One thread group (mnt6_cgbn_params_t::TPI threads cooperate)
 */
template<class params>
__global__ void msm_naive_cgbn_kernel(
    ...
    // Instantiate MNT6 MSM class
    mnt6_msm_t<params> msm(cgbn_report_monitor, report, instance);
    typename mnt6_msm_t<params>::bn_t P, px, py;
    typename mnt6_msm_t<params>::bn_t acc_x, acc_y, acc_zz, acc_zzz;
    typename mnt6_msm_t<params>::bn_t term_x, term_y, term_zz, term_zzz;

    // Load modulus
    cgbn_load(msm._env, P, (cgbn_mem_t<params::BITS>*)MNT6_298_P_DEVICE);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (point_is_infinity) {
            ...
            if (scalar_is_zero) {
                ...
                if (highest_bit < 0) {
                    ...
                    typename mnt6_msm_t<params>::bn_t dbl_x, dbl_y, dbl_zz, dbl_zzz;
                    typename mnt6_msm_t<params>::bn_t add_x, add_y, add_zz, add_zzz;
        ...
        if (!term_is_infinity) {
            if (acc_is_infinity) {
                ...
                typename mnt6_msm_t<params>::bn_t new_x, new_y, new_zz, new_zzz;
                ...
                typename mnt6_msm_t<params>::bn_t cross1, cross2, cross3, cross4;
                ...
                typename mnt6_msm_t<params>::bn_t y_sum;
...
/**
 * FFI Entry Point: CGBN-based MSM for MNT6-298 G1
 *
 ...
 */
extern "C" int msm_mnt6_298_g1_cgbn(
    ...
    if (ffi_affine_sz != sizeof(affine_cgbn_t)) {
        fprintf(stderr, "[MNT6-298 CGBN] ERROR: Affine size mismatch: CUDA expects %zu, Rust provided %zu\n",
               sizeof(affine_cgbn_t), ffi_affine_sz);
        return MNT6_MSM_ERROR_AFFINE_LAYOUT;
    }

    if (ffi_scalar_sz != sizeof(scalar_cgbn_t)) {
        fprintf(stderr, "[MNT6-298 CGBN] ERROR: Scalar size mismatch: CUDA expects %zu, Rust provided %zu\n",
               sizeof(scalar_cgbn_t), ffi_scalar_sz);
        return MNT6_MSM_ERROR_SCALAR_LAYOUT;
    }
    ...
    if (err != cudaSuccess) return MNT6_MSM_ERROR_CUDA_RUNTIME;

    err = cudaMalloc(&d_scalars, sizeof(scalar_cgbn_t) * count);
    if (err != cudaSuccess) {
        cudaFree(d_points);
        return MNT6_MSM_ERROR_CUDA_RUNTIME;
    }
    ...
    threads_per_block = mnt6_cgbn_params_t::TPI;
    num_blocks = 1;

    msm_naive_cgbn_kernel<mnt6_cgbn_params_t><<<num_blocks, threads_per_block>>>(
        d_report, d_result, d_points, d_scalars, count
    );

    err = cudaDeviceSynchronize();
    if (err != cudaSuccess) {
        fprintf(stderr, "[MNT6-298 CGBN] ERROR: Kernel failed: %s\n", cudaGetErrorString(err));
        goto cleanup_error;
    }

    // Check for CGBN errors
    if (cgbn_error_report_check(d_report)) {
        fprintf(stderr, "[MNT6-298 CGBN] ERROR: CGBN error detected\n");
        goto cleanup_error;
    }
    ...
    if (err != cudaSuccess) {
        fprintf(stderr, "[MNT6-298 CGBN] ERROR: Failed to copy result: %s\n", cudaGetErrorString(err));
        goto cleanup_error;
    }
...
uint32_t compute_optimal_wbits_mnt6(uint32_t n) {
...
__host__ __device__ __forceinline__
uint32_t compute_nwins_mnt6(uint32_t scalar_bits, uint32_t wbits) {
...
} __align__(8) bucket_xyzz_mnt6_t;

// ============================================================================
// KERNEL 1: Scalar Breakdown (extract signed digits)
// ============================================================================
__global__ void breakdown_scalars_mnt6_298(
...
// ============================================================================
__global__ void histogram_buckets_mnt6_298(
...
// ============================================================================
__global__ void prefix_sum_histogram_mnt6_298(
...
// ============================================================================
__global__ void scatter_to_buckets_mnt6_298(
...
// ============================================================================
template<class params>
__global__ void accumulate_buckets_mnt6_298(
    cgbn_error_report_t* report,
    bucket_xyzz_mnt6_t* buckets,    // Output: [nwins * num_buckets]
    ...
    typename mnt6_msm_t<params>::bn_t P, px, py;
    typename mnt6_msm_t<params>::bn_t acc_x, acc_y, acc_zz, acc_zzz;
    typename mnt6_msm_t<params>::bn_t tmp_x, tmp_y, tmp_zz, tmp_zzz;

    // Load modulus (MNT6-298 base field)
    cgbn_load(msm._env, P, (cgbn_mem_t<params::BITS>*)MNT6_298_P_DEVICE);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (negate) {
            typename mnt6_msm_t<params>::bn_t neg_y;
...
// ============================================================================
template<class params>
__global__ void integrate_buckets_mnt6_298(
    cgbn_error_report_t* report,
    bucket_xyzz_mnt6_t* window_sums,     // Output: [nwins]
    const bucket_xyzz_mnt6_t* buckets,   // Input: [nwins * num_buckets]
    uint32_t nwins,
    uint32_t wbits
) {
    int32_t instance = (blockIdx.x * blockDim.x + threadIdx.x) / params::TPI;
    ...
    // Initialize CGBN
    mnt6_msm_t<params> msm(cgbn_report_monitor, report, instance);
    typename mnt6_msm_t<params>::bn_t P;
    typename mnt6_msm_t<params>::bn_t acc_x, acc_y, acc_zz, acc_zzz;
    typename mnt6_msm_t<params>::bn_t sum_x, sum_y, sum_zz, sum_zzz;
    typename mnt6_msm_t<params>::bn_t tmp_x, tmp_y, tmp_zz, tmp_zzz;
    typename mnt6_msm_t<params>::bn_t bkt_x, bkt_y, bkt_zz, bkt_zzz;
    ...
    for (int32_t b = num_buckets - 1; b >= 1; b--) {
        ...
        if (!buckets[bkt_idx].is_infinity) {
            ...
            if (acc_is_infinity) {
                ...
                typename mnt6_msm_t<params>::bn_t lhs, rhs;
...
// ============================================================================
template<class params>
__global__ void combine_windows_mnt6_298(
    cgbn_error_report_t* report,
    jacobian_cgbn_t* result_out,
    const bucket_xyzz_mnt6_t* window_sums,
    uint32_t nwins,
    uint32_t wbits
) {
    int32_t instance = (blockIdx.x * blockDim.x + threadIdx.x) / params::TPI;
    ...
    mnt6_msm_t<params> msm(cgbn_report_monitor, report, instance);
    typename mnt6_msm_t<params>::bn_t P;
    typename mnt6_msm_t<params>::bn_t res_x, res_y, res_zz, res_zzz;
    typename mnt6_msm_t<params>::bn_t tmp_x, tmp_y, tmp_zz, tmp_zzz;
    typename mnt6_msm_t<params>::bn_t win_x, win_y, win_zz, win_zzz;
...
// ============================================================================
// FFI Entry Point: Pippenger MSM for MNT6-298 G1
// ============================================================================
extern "C" int msm_mnt6_298_g1_cgbn_pippenger(
    const void* points_ptr,
    const void* scalars_ptr,
    size_t count,
    void* result_ptr,
    ...
    // Validate FFI layout
    if (ffi_affine_sz != sizeof(affine_cgbn_t)) {
        fprintf(stderr, "[MNT6-298 Pippenger] ERROR: Affine size mismatch\n");
        return MNT6_MSM_ERROR_AFFINE_LAYOUT;
    }
    if (ffi_scalar_sz != sizeof(scalar_cgbn_t)) {
        fprintf(stderr, "[MNT6-298 Pippenger] ERROR: Scalar size mismatch\n");
        return MNT6_MSM_ERROR_SCALAR_LAYOUT;
    }
    ...
    if (count < 64) {
        return msm_mnt6_298_g1_cgbn(points_ptr, scalars_ptr, count,
    ...
    uint32_t wbits = compute_optimal_wbits_mnt6(n);
    uint32_t nwins = compute_nwins_mnt6(PIPPENGER_SCALAR_BITS, wbits);
    uint32_t num_buckets = (1u << (wbits - 1)) + 1;
    uint32_t total_buckets = nwins * num_buckets;
    ...
    bucket_xyzz_mnt6_t* d_buckets = nullptr;
    bucket_xyzz_mnt6_t* d_window_sums = nullptr;
    jacobian_cgbn_t* d_result = nullptr;
    cgbn_error_report_t* d_report = nullptr;

    // Allocation
    err = cudaMalloc(&d_points, sizeof(affine_cgbn_t) * n);
    if (err != cudaSuccess) goto pippenger_cleanup_error_mnt6;
    ...
    err = cudaMalloc(&d_buckets, sizeof(bucket_xyzz_mnt6_t) * total_buckets);
    if (err != cudaSuccess) goto pippenger_cleanup_error_mnt6;

    err = cudaMalloc(&d_window_sums, sizeof(bucket_xyzz_mnt6_t) * nwins);
    if (err != cudaSuccess) goto pippenger_cleanup_error_mnt6;
...

    {
        ...
        // 1. Breakdown scalars
        breakdown_scalars_mnt6_298<<<blocks_n, threads>>>(
        ...
        // 2. Build histogram
        histogram_buckets_mnt6_298<<<blocks_n, threads>>>(
        ...
        prefix_sum_histogram_mnt6_298<<<blocks_nwins, threads>>>(
        ...
        scatter_to_buckets_mnt6_298<<<blocks_n, threads>>>(
        ...
        int tpi = mnt6_cgbn_params_t::TPI;
        ...
        accumulate_buckets_mnt6_298<mnt6_cgbn_params_t><<<blocks_accumulate, threads_accumulate>>>(
        ...
        integrate_buckets_mnt6_298<mnt6_cgbn_params_t><<<blocks_integrate, threads_accumulate>>>(
        ...
        combine_windows_mnt6_298<mnt6_cgbn_params_t><<<1, tpi>>>(
    ...
    // Check CGBN errors
    if (cgbn_error_report_check(d_report)) {
        fprintf(stderr, "[MNT6-298 Pippenger] ERROR: CGBN error detected\n");
        goto pippenger_cleanup_error_mnt6;
    }
