/***
 * Standalone CGBN Field Arithmetic Test for MNT6-298
 *
 * Purpose: Validate that CGBN can correctly perform field operations
 *          for MNT6-298's 298-bit base field before attempting integration.
 *
 * Note: MNT4/MNT6 form a cycle pair:
 *   - MNT6-298 Fq == MNT4-298 Fr
 *   - MNT6-298 Fr == MNT4-298 Fq
 *
 * Strategy:
 ...
 *
 * Compile: nvcc -o test_cgbn_mnt6_field test_cgbn_mnt6_field.cu -I../../cgbn-lib/include -lgmp -arch=sm_75
 * Run: ./test_cgbn_mnt6_field
 */
...
// CGBN configuration for MNT6-298
...
// From msm_mnt6_298_cgbn.cu
__device__ __constant__ uint32_t MNT6_298_MODULUS[10] = {
...
uint32_t MNT6_298_MODULUS_HOST[10] = {
    0x00000001, 0xbb4334a4, 0x925d6ad3, 0xfb494c07, 0x5cf44194,
    0xcaeec963, 0xa7b0548e, 0xa266249d, 0xf7bcd473, 0x000003bc
};
...
test_instance_t* generate_test_instances(uint32_t count) {
    ...
    gmp_randseed_ui(rng, 67890);  // Different seed from MNT4 test

    // Load modulus
    limbs_to_mpz(modulus, MNT6_298_MODULUS_HOST, 10);

    printf("MNT6-298 modulus (hex): ");
    gmp_printf("%Zx\n", modulus);
...
__global__ void test_field_ops(cgbn_error_report_t *report, test_instance_t *instances, uint32_t count) {
    ...
    // Load modulus
    cgbn_load(bn_env, modulus, (cgbn_mem_t<BITS>*)MNT6_298_MODULUS);
...
int main() {
    ...
    printf("=== CGBN MNT6-298 Field Arithmetic Test ===\n");
    ...
    if(all_passed) {
        printf("\n[SUCCESS] All MNT6-298 field arithmetic tests passed!\n");
