// Test suite for MNT6-298 Pippenger MSM implementation
...
#[cfg(feature = "gpu")]
use sppark_msm::{msm_mnt6_298_gpu_cgbn, msm_mnt6_298_gpu_pippenger};
...
#[test]
#[cfg(feature = "gpu")]
fn test_pippenger_mnt6_vs_serial() {
    let n = 256;
    let (points, scalars) = generate_random_points_and_scalars(n, 11111);

    let serial_result = msm_mnt6_298_gpu_cgbn(&points, &scalars)
...
#[test]
#[cfg(feature = "gpu")]
fn test_pippenger_mnt6_empty() {
    let points: Vec<G1Affine> = vec![];
    let scalars: Vec<BigInt<5>> = vec![];

    let result = msm_mnt6_298_gpu_pippenger(&points, &scalars)
...
#[test]
#[cfg(feature = "gpu")]
fn test_pippenger_mnt6_single_point() {
...
#[test]
#[cfg(feature = "gpu")]
fn test_pippenger_mnt6_zero_scalars() {
    let n = 10;
    let mut rng = StdRng::seed_from_u64(12345);
    let points: Vec<G1Affine> = (0..n)
        .map(|_| G1Projective::rand(&mut rng).into_affine())
        .collect();
    let scalars: Vec<BigInt<5>> = vec![BigInt::<5>::zero(); n];
...
#[test]
#[cfg(feature = "gpu")]
fn test_pippenger_mnt6_deterministic() {
    let n = 100;
    let (points, scalars) = generate_random_points_and_scalars(n, 99999);

    let result1 = msm_mnt6_298_gpu_pippenger(&points, &scalars)
        .expect("Pippenger MSM failed");
    let result2 = msm_mnt6_298_gpu_pippenger(&points, &scalars)
        .expect("Pippenger MSM failed");
    let result3 = msm_mnt6_298_gpu_pippenger(&points, &scalars)
        .expect("Pippenger MSM failed");
...
#[test]
#[cfg(feature = "gpu")]
fn test_pippenger_mnt6_sweep_sizes() {
...
#[test]
#[cfg(feature = "gpu")]
fn test_pippenger_mnt6_identical_scalars() {
...
#[cfg(feature = "gpu")]
fn benchmark_pippenger_vs_serial_mnt6() {
    for n in [1000, 2000, 5000, 10000] {
        let (points, scalars) = generate_random_points_and_scalars(n, 12345);

        // Warmup
        let _ = msm_mnt6_298_gpu_cgbn(&points, &scalars);
        let _ = msm_mnt6_298_gpu_pippenger(&points, &scalars);
