// CGBN-based MNT4-298 GPU MSM Tests
// Custom test harness (harness = false) to avoid Rust test framework CUDA cleanup conflicts
//
// Run with: cargo test --release --features gpu --test test_gpu_cgbn_mnt4
...
fn call_cgbn_msm(
    ...
    match msm_mnt4_298_gpu_cgbn(points, scalars) {
        Ok(result) => Ok(result),
        Err(_) => Err(-1),
    }
}
...
fn main() {
    println!("╔════════════════════════════════════════════════════╗");
    println!("║  CGBN MNT4-298 GPU MSM Tests (Custom Harness)     ║");
    println!("╚════════════════════════════════════════════════════╝");
